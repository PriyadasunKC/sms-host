...
const UserApplication = () => {
  ...
  const handleStatus = async (id, status) => {
    try {
      ...
      const res = await axios.post(`${baseUrl}/api/v1/admin/handle-status`, {
        id: id,
        status: status,
      });
    ...
    try {
      ...
      const aproveResponse = await axios.post(
        `${baseUrl}/api/v1/aprove/aprove-user-model`,
        {
          email: location.state.record.Email,
          userRole: location.state.record.UserRole,
        }
      );
  ...
  const handleUpdatedDetails = async (updatedId) => {
    try {
      const UpdatedUser = await axios.patch(
        `${baseUrl}/api/v1/admin/update-details`,
        {
          updatedId: updatedId,
          FirstName: FirstName,
          LastName: LastName,
  ...
  return (
    <SideBar>
      {location.state.record.status === "pending" ? (
            ...
            <div>
              ...
              >
                ...
                <input
                  ...
                  name="category"
                  value={
                    location.state?.record?.catagory
                      ? location.state?.record?.catagory
                      : "Not selected"
                  }
                  readOnly={true}
        ...
        >
          <div style={{}} className="UserApplicationForm">
            <div
              style={{
                backgroundColor: "#15295E",
              }}
