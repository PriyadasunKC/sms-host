...
const RefereeEvent = () => {
  ...
  const fetchData = async () => {
    let data = await fetch(`${baseUrl}/api/v1/event/get-all-events`);
  ...
  const fetchSelectedEvents = async () => {
    let data = await fetch(`${baseUrl}/api/v1/referee/get-all-referee-events`);
    data = await data.json();
    console.log(data.data);
  ...
  const addRefereeAvailability = async (ref_id, event_id) => {
    try {
      const response = await fetch(
        `${baseUrl}/api/v1/referee/create-referee-event`,
        {
          method: "POST",
          headers: {
  ...
  const removeRefereeAvailability = async (id) => {
    try {
      const response = await fetch(
        `${baseUrl}/api/v1/referee/delete-referee-event/${id}`,
        {
          method: "DELETE",
        }
      );
