...
const CoachReviewPlayers = () => {
  ...
  const fetchData = async (page) => {
    try {
      const response = await axios.post(`${baseUrl}/api/v1/player/player-pagination`, { page });
  ...
  const handlePlayerNameSearch = async(value) => {
    console.log(value);
    try {
      const searchResult = await axios.post(`${baseUrl}/api/v1/player/search-player`,{playerName:value});
