...
const SelectPlayers = () => {
  ...
  const getOnlyAvailablePlayer = async () => {
    try {
      const availablePlayer = await axios.post(
        `${baseUrl}/api/v1/player-availability/getEventPlayer`,
        { eventId: matchId }
      );
  ...
  useEffect(() => {
    axios
      .get(
        `${baseUrl}/api/v1/coach/players?match_id=${matchId}&coach_id=${coachId}`
      )
      .then((res) => {
  ...
  const handleSubmit = async () => {
    try {
      const res = await axios.post(
       `${baseUrl}/api/v1/coach/create-team`,
        teamData
      );
      if (res.data.success) {
  ...
  return (
    <CoachSidebar>
      ...
      >
        <Form
          className="text"
          style={{ margin: 150, marginBottom: 80, marginTop: "0px" }}
        >
        ...
        <Table
          ...
          dataSource={players}
          style={{ margin: 50, marginTop: 10, marginBottom: 0 }}
        />
