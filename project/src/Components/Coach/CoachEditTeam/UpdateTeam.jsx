...
const UpdateTeam = () => {
    ...
    useEffect(() => {
        axios
          .get(
            `${baseUrl}/api/v1/coach/players?match_id=${team.match_id}&coach_id=${team.coach_id}`
          )
          .then((res) => {
    ...
    const getOnlyAvailablePlayer = async()=>{
        try {
          const availablePlayer = await axios.post(`${baseUrl}/api/v1/player-availability/getEventPlayer` , {eventId:team.match_id}) 
    ...
    const handleSubmit = async () => {
        try {
            ...
            const res = await axios.post(
              `${baseUrl}/api/v1/coach/update-team`,
              { teamData: teamData }
            );
            if (res.data.success) {
