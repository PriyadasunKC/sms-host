...
const TeamManagerAssignCoaches = () => {
  ...
  const getFetchData = async () => {
    let data = await fetch(
      `${baseUrl}/api/v1/coaches-assign-delete/get-coachesassignee`
    );
  ...
  useEffect(() => {
    axios.defaults.baseURL = `${baseUrl}/api/v1/coaches-assign-delete`;
    getFetchData();
  }, []);
                      ...
                      return (
                        <>
                          <button
                            className="assignBtn"
                            onClick={() =>
                              handleAssign(record._id, record.status)
                            }
                            disabled={assignedIds.includes(record._id)}
                          >
