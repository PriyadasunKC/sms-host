...
const TeamManagerAssignCoaches = () => {
  ...
  useEffect(() => {
    axios.defaults.baseURL =
      `${baseUrl}/api/v1/Assign-Coaches-New`;
    getFetchData();
  }, []);
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (confirmed) {
        console.log(id);
        const response = await axios.post("/delete-assigncoachesstatus", { id: id });
    ...
    try {
      ...
      if (confirmed) {
        ...
        if (!status) {
          ...
          await axios
            .post("/change-assigncoachesstatus", { coachId, eventId, task: "assign" }) // Adjust the endpoint URL accordingly
            .then((res) => {
              if (res.data.success) {
                // setIsAssigned(true)
