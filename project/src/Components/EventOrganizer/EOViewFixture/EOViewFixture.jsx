...
const EOViewFixture = () => {
  ...
  const fetchData = async (page) => {
    try {
      const response = await axios.post(`${baseUrl}/api/v1/get/pagination`, { page });
  ...
  const getFixtureData = async () => {
    try {
      const response = await axios.get(
        `${baseUrl}/api/v1/get/get-fixture`
      );
  ...
  const handleEventLocationSearch = async (value) => {
    ...
    try {
      const searchResponse = await axios.post(`${baseUrl}/api/v1/search/data`, { value: value, searchDate: searchDate })
