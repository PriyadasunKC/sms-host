...
export default function EOAssignRefereeFinal() {
  ...
  console.log(location);
    ...
    const getAvailableReferee = async () => {
         try {
            const availabilityEventResponse = await axios.post(`${baseUrl}/api/v1/availability/event-available-referee`,{eventId:location.state.eventId})
    ...
    const handleRefereeAdd = async(refereeId)=>{
        try {
                const addCoaches = await axios.post(`${baseUrl}/api/v1/event/assignReferee`,{eventId:location.state.eventId ,refereeId:refereeId})
