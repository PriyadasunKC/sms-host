...
const EOCommunicationToTM = () => {
  ...
  const fetchData = async (page) => {
    try {
      const response = await axios.post(`${baseUrl}/api/v1/team-manager/pagination`, { page });
  ...
  const getOnlyCoach = async () => {
    try {
      const response = await axios.get(`${baseUrl}/api/v1/team-manager/details`)
  ...
  const handleEventNameSearch = async(value) => {
    ...
    try {
      const searchResponse = await axios.post(`${baseUrl}/api/v1/search/filterTM`,{TMName:value})
                    ...
                    render: (text, record) => (
                      ...
                      >
                        <Button
                          type="primary"
                          onClick={() => {
                            navigate("/eo-communication-to-tm-form",{state:{record:record}});
                          }}
                          style={{
