...
const EOCommunicationToCoach = () => {
  ...
  const fetchData = async (page) => {
    try {
      const response = await axios.post(`${baseUrl}/api/v1/coach/pagination`, { page });
  ...
  const handleEventNameSearch = async(value) => {
    ...
    try {
      const searchResponse = await axios.post(`${baseUrl}/api/v1/search/filterCoaches`,{coachName:value})
