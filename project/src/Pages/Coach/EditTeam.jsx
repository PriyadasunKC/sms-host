...
const EditTeam = () => {
  ...
  useEffect(() => {
    axios
      .get(`${baseUrl}/coach/teams?coach_id=${coach_id}`)
      .then((res) => {
  ...
  const handleEdit = (team_id, match_id, teamName, teamNo, players) => {
    navigate(
      `/update-team?team_id=${team_id}&match_id=${match_id}&coach_id=${coach_id}&teamName=${teamName}&teamNo=${teamNo}&players=${players}`
    );
  };
  ...
  const handleDelete = async (team_id, coach_id, match_id) => {
    try {
      const res = await axios.delete(
        `${baseUrl}/coach/delete-team?team_id=${team_id}&coach_id=${coach_id}&match_id=${match_id}`
      );
      if (res.data.success) {
            ...
            {teams.map((team, index) => (
              <tr key={index}>
                ...
                <td style={{ textAlign: "center" }}>
                  ...
                  <button
                    onClick={() =>
                      handleDelete(team._id, team.coach_id, team.match_id)
                    }
                  >
                    Delete
                  </button>
