...
const TournamentBracket = () => {
  ...
  useEffect(() => {
    axios
      .get(`${baseUrl}/api/v1/eventOrganizerBracket/getTeams`)
      .then((res) => {
  ...
  const pairsGet = async () => {
    try {
      ...
      const response = await axios.get(
        `${baseUrl}/create-round/${selectedMatchId}/${roundNo}`
      );
  ...
  const handleNextRound = () => {
    axios
      .post(`${baseUrl}/setWinners/${selectedMatchId}/${roundNo}`, {
        winnersArray,
      })
      .then((res) => {
  ...
  const handleViewWinners = async () => {
    try {
      const res = await axios.get(`${baseUrl}/getWinners/${selectedMatchId}`);
  ...
  return (
    <div
      style={{ margin: 10, padding: 10, height: "100%", overflow: "scroll" }}
    >
      <div
        style={{
      ...
      >
        <button style={{}} onClick={handleViewWinners}>
          View winners
        </button>
        {
